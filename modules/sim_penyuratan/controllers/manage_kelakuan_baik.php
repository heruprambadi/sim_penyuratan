...
/**
 * Description of Manage_Kelakuan_Baik
 *
 * @author No-CMS Module Generator
 */
class Manage_Kelakuan_Baik extends CMS_Priv_Strict_Controller {
    ...
    public function index(){
        ...
        // set model
        // $crud->set_model($this->cms_module_path().'/grocerycrud_kelakuan_baik_model');

        // adjust groceryCRUD's language to No-CMS's language
        $crud->set_language($this->cms_language());

        // table name
        $crud->set_table($this->cms_complete_table_name('kelakuan_baik'));

        // set subject
        $crud->set_subject('Surat Ket. Kelakuan Baik');

        // displayed columns on list
        $crud->columns('nomor_surat','nama_mahasiswa','npm');
        $crud->order_by('id_kelakuan_baik', 'DESC');
        ...
        //Add Action
        $crud->add_action('Download', base_url().'/assets/icon/word.png', base_url().'sim_penyuratan/manage_kelakuan_baik/generate/');
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $output = $crud->render();
        $this->view($this->cms_module_path().'/manage_kelakuan_baik_view', $output,
            $this->cms_complete_navigation_name('manage_kelakuan_baik'));
    ...
    public function before_insert_update($post_array){
        ...
        $data = $this->db->select('MAX(nomor_surat) max, MIN(nomor_surat) min, bulan, tahun')
                         ->where('bulan',$bulan)
                         ->where('tahun',$tahun)
                         ->get($this->cms_complete_table_name('kelakuan_baik'))->row();
        if($data->min != 1){
    ...
    public function generate($id=0){
        ...
        $data = $this->db->where('id_kelakuan_baik',$id)
                         ->join($this->cms_complete_table_name('mas_jurusan'), 'id_jurusan=fk_id_jurusan')
                         ->get($this->cms_complete_table_name('kelakuan_baik'))->result();
        $PHPWord = new PHPWord();
