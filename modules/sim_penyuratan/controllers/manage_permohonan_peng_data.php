 ...
 */
class Manage_Permohonan_Peng_Data extends CMS_Priv_Strict_Controller {
    ...
    public function index(){
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $output = $crud->render();
        $this->view($this->cms_module_path().'/manage_permohonan_peng_data_view', $output,
            $this->cms_complete_navigation_name('manage_permohonan_peng_data'));
    ...
    public function before_insert_update($post_array){
        ...
        $data = $this->db->select('MAX(nomor_surat) max, MIN(nomor_surat) min, bulan, tahun')
                         ->where('bulan',$bulan)
                         ->where('tahun',$tahun)
                         ->get($this->cms_complete_table_name('permohonan_peng_data'))->row();
        if($data->min != 1){
    ...
    public function generate($id=0){
        ...
        //if ($id==0);
        $data = $this->db->where('id_permohonan_peng_data',$id)
                         ->join($this->cms_complete_table_name('mas_jurusan'), 'id_jurusan=fk_id_jurusan')
                         ->get($this->cms_complete_table_name('permohonan_peng_data'))->result();
        $PHPWord = new PHPWord();
