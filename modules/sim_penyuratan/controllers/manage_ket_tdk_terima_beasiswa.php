...
/**
 * Description of Manage_Ket_Tdk_Terima_Beasiswa
 *
 * @author No-CMS Module Generator
 */
class Manage_Ket_Tdk_Terima_Beasiswa extends CMS_Priv_Strict_Controller {
    ...
    public function index(){
        ...
        // set model
        // $crud->set_model($this->cms_module_path().'/grocerycrud_ket_tdk_terima_beasiswa_model');

        // adjust groceryCRUD's language to No-CMS's language
        $crud->set_language($this->cms_language());

        // table name
        $crud->set_table($this->cms_complete_table_name('ket_tdk_terima_beasiswa'));

        // set subject
        $crud->set_subject('Surat Ket. Tidak Menerima Beasiswa');

        // displayed columns on list
        $crud->columns('no_surat','nama_mahasiswa','npm');
        $crud->order_by('id_ket_tdk_terima_beasiswa', 'DESC');
        // displayed columns on edit operation
        $crud->edit_fields('no_surat','nama_mahasiswa','npm','tempat_lahir','tanggal_lahir','fk_id_jurusan','semester','tahun_akademis','bulan','tahun');
        // displayed columns on add operation
        $crud->add_fields('no_surat','nama_mahasiswa','npm','tempat_lahir','tanggal_lahir','fk_id_jurusan','semester','tahun_akademis','bulan','tahun');
        
        //Add Action
        $crud->add_action('Download', base_url().'/assets/icon/word.png', base_url().'sim_penyuratan/manage_ket_tdk_terima_beasiswa/generate/');
        ...
        $crud->field_type('no_surat','invisible');
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $output = $crud->render();
        $this->view($this->cms_module_path().'/manage_ket_tdk_terima_beasiswa_view', $output,
            $this->cms_complete_navigation_name('manage_ket_tdk_terima_beasiswa'));
    ...
    public function before_insert_update($post_array){
        $bulan = date('m');
        $tahun = date('Y');
        $data = $this->db->select('MAX(no_surat) max, MIN(no_surat) min, bulan, tahun')
                         ->where('bulan',$bulan)
                         ->where('tahun',$tahun)
                         ->get($this->cms_complete_table_name('ket_tdk_terima_beasiswa'))->row();
        if($data->min != 1){
            $post_array['no_surat'] = 1;
        }else{
            $post_array['no_surat'] = $data->max+1;
        }
        return $post_array;
    }
    ...
    public function generate($id=0){
        ...
        $data = $this->db->where('id_ket_tdk_terima_beasiswa',$id)
                         ->join($this->cms_complete_table_name('mas_jurusan'), 'id_jurusan=fk_id_jurusan')
                         ->get($this->cms_complete_table_name('ket_tdk_terima_beasiswa'))->result();
        $PHPWord = new PHPWord();

        //Generate Document
        foreach($data as $d):
        $document = $PHPWord->loadTemplate('assets/surat_keluar/Ket. Tidak Menerima Beasiswa/tidak_menerima_bs.docx');
        $document->setValue('nomor_surat', $d->no_surat);
        ...
        $filename = $d->npm.'-Ket_Tdk_Terima_Bs.docx';
