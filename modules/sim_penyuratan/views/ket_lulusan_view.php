...
<script type="text/javascript">
    ...
    for(var i=0; i<old_data.length; i++){
        var row = old_data[i];
        var record_index = i;
        var primary_key = row['id_keterangan_lulusan'];
        var data = row;
        delete data['id_keterangan_lulusan'];
    ...
    function add_table_row_citizen(value){
        ...
        component += '<td>';
        component += '<input id="md_field_citizen_col_name_'+RECORD_INDEX_citizen+'" record_index="'+RECORD_INDEX_citizen+'" class="md_field_citizen_col" column_name="keterangan" type="text" value="'+field_value+'"/>';
        component += '</td>';
    ...
    $(document).ajaxSuccess(function(event, xhr, settings) {        
        if (settings.url == "{{ module_site_url }}manage_ket_lulusan/index/insert") {
